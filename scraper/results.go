...
func CreateResult(db *sql.DB, gender string, details []string, raceSummary map[string]string) int {
	...
	var id int
	runner_id, err := FindRunner(db, details[0], details[1])
	if err == sql.ErrNoRows {
		fmt.Println("No Runner found, creating a new one")
		// Order is last name, first name
		runner_id = AddRunner(db, details[1], details[0])
		// fmt.Println(runner_id)
	}
	...
	if err == sql.ErrNoRows {
		fmt.Println("No team found, creating a new one")
	...
	if err == sql.ErrNoRows {
		fmt.Println("This Race does not exist, creating a new RACE")
		race_id = AddRace(db, raceSummary, gender)
	}
	fmt.Printf("Race ID: %d\n", race_id)
	...
	result_id, err := FindResult(db, details[4], raceSummary["distance"], runner_id, instance_id)
	if err == sql.ErrNoRows {
		fmt.Println("Adding Result")
		result_id = AddResult(db, details[4], raceSummary["distance"], runner_id, instance_id)
	}
